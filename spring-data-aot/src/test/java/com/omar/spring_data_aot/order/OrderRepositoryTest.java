...
class OrderRepositoryTest {
    ...
    void findByCustomerName_shouldFindOrdersForCustomer() {
        ...
        assertThat(results).allMatch(order ->
            order.customerName().equals("Alice Johnson"));
    }
    ...
    void findByStatusAndOrderDateAfter_shouldFilterByStatusAndDate() {
        ...
        List<Order> results = orderRepository.findByStatusAndOrderDateAfter(
            OrderStatus.PENDING, oneWeekAgo);

        assertThat(results).isNotEmpty();
        assertThat(results).allMatch(order ->
            order.status() == OrderStatus.PENDING &&
            order.orderDate().isAfter(oneWeekAgo));
    }
    ...
    void findByStatusAndOrderDateAfter_shouldReturnEmptyForFutureDate() {
        ...
        List<Order> results = orderRepository.findByStatusAndOrderDateAfter(
            OrderStatus.PENDING, futureDate);
